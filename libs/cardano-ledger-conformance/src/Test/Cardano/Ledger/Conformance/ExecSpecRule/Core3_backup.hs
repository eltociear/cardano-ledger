...
conformsToImpl =
  forAllShow genCtx showExpr $ \ctx -> do
    let (genEnv, shrinkEnv, genState, genSignal) = genSTSTypes @fn @rule @era ctx
    forAllShrinkShow genEnv shrinkEnv showExpr $ \env ->
...
class
  ...
  genSTSTypes ::
    ...
    , Environment (EraRule rule era) -> [Environment (EraRule rule era)]
    , Gen (State (EraRule rule era))
    , -- , State (EraRule rule era) -> [State (EraRule rule era)]
