...
protocolVersion_ :: (HasSpec fn Coin, BaseUniverse fn) => Term fn SimplePParams -> Term fn ProtVer
protocolVersion_ = sel @13

maxTxSize_ :: (HasSpec fn Coin, BaseUniverse fn) => Term fn SimplePParams -> Term fn Word32
maxTxSize_ = sel @3
...
data SimplePParams = SimplePParams
  { minFeeA :: Coin
  , minFeeB :: Coin
  , maxBBSize :: Word32
  , maxTxSize :: Word32
...
instance HasSimpleRep SimplePParams

instance BaseUniverse fn => HasSpec fn SimplePParams
...
-- \| EraPP era means we can go back and forth between SimplePParams and (PParams era)
--   This allow us to use SimplePParams as the (SimpleRep (PParams era))
--   Much easier to constrain SimplePParams than (PParams era) with all the THKD stuff.
class
  ( Era era
  ...
  EraPP era
  where
  subsetToPP :: SimplePParams -> PParams era
  ppToSubset :: PParams era -> SimplePParams
...
dropAtMost6 ::
  (EraPParams era, ProtVerAtMost era 6) => PParams era -> SimplePParams -> SimplePParams
...
dropShelley :: EraPParams era => PParams era -> SimplePParams
dropShelley pp =
  SimplePParams
...
dropAlonzo :: AlonzoEraPParams era => PParams era -> SimplePParams -> SimplePParams
dropAlonzo pp psub =
  psub
    { coinsPerUTxOWord = Coin 0
...
dropBabbage :: BabbageEraPParams era => PParams era -> SimplePParams -> SimplePParams
dropBabbage pp psub =
  psub {coinsPerUTxOByte = unCoinPerByte (pp ^. ppCoinsPerUTxOByteL)}

dropConway :: ConwayEraPParams era => PParams era -> SimplePParams -> SimplePParams
dropConway pp psub =
  psub
...
liftShelley :: EraPParams era => SimplePParams -> PParams era
liftShelley pps =
  emptyPParams
...
liftAlonzo :: AlonzoEraPParams era => SimplePParams -> PParams era -> PParams era
liftAlonzo pps pp =
...
liftBabbage :: BabbageEraPParams era => SimplePParams -> PParams era -> PParams era
liftBabbage pps pp = pp & ppCoinsPerUTxOByteL .~ CoinPerByte (coinsPerUTxOByte pps)

liftConway :: ConwayEraPParams era => SimplePParams -> PParams era -> PParams era
liftConway pps pp =
  pp
...
instance EraPP era => HasSimpleRep (PParams era) where
  type SimpleRep (PParams era) = SimplePParams
